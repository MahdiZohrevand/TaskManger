...
class DateTest {
    ...
    /**
     * birthday date in millisecond
     */
    private val birthDayDate = LocalDate.of(1971, 6, 24)
    ...
    fun getPeriod() {
        ...
        print(
            "birth date  ${birthDate.get(Calendar.YEAR)} , ${birthDate.get(Calendar.MONTH)} , ${birthDate.get(
    ...
    @Test
    fun `age between two date in millisecond format`() {
        val birthDateInMillisecond = helper.convertPersianDateToMillisecond(year, month, day)
        val nowInMillisecond = Calendar.getInstance().timeInMillis
        val ageInMillisecond = nowInMillisecond - birthDateInMillisecond
        println(ageInMillisecond)
        val date = Date(ageInMillisecond)
        println("${date.year}")
    }
